...
public class DS4
{
    ...
    public static Gamepad getConroller(string layoutFile = null)
    {
        ...
        string layout = File.ReadAllText(layoutFile == null ? "Assets/Emmas-Assets/Scripts/customLayout.json" : layoutFile);

        // Overwrite the default layout
        InputSystem.RegisterLayoutOverride(layout, "MyDualShock4GamepadHID");
    ...
    {
        float x = processRawData(acclX.ReadValue());
        float y = processRawData(acclY.ReadValue());
        float z = processRawData(acclZ.ReadValue());
